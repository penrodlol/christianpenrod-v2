...
export const Code: FC<CodeProps> = ({ title, children }) => {
  ...
  return (
    <>
      {title && (
        <div className="ml-auto mr-4 max-w-max rounded-t-md bg-[hsl(231.4,20%,6.9%)] py-0.5 px-3 elevation-10">
          <span className="text-2 text-sm">{title}</span>
        </div>
      )}
      <div className="relative rounded-md bg-[hsl(231.4,20%,6.9%)] elevation-10">
