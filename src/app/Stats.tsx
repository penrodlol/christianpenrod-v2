  ...
  return (
    <div className="flex flex-wrap justify-between gap-x-4 gap-y-1 rounded-md bg-2 py-4 px-6 text-sm elevation-10">
      ...
      <div className="flex flex-col gap-1">
        ...
        <Stat label="Top Language:" data={'TypeScript'} icon={<Code2 />} />
      </div>
    </div>
  );
};
...
export const getUser = async () => {
  ...
  const query = `query { user(login: "${env.GITHUB_USERNAME}") { contributions: contributionsCollection(from: "${from}") { totalCommitContributions restrictedContributionsCount } reposForStars: repositories(ownerAffiliations: OWNER last: 100 orderBy: { direction: DESC, field: STARGAZERS }) { nodes { stargazers { totalCount } } } pullRequests(first: 1) { totalCount } issues { totalCount } contributedTo: repositoriesContributedTo(first: 1 contributionTypes: [COMMIT, ISSUE, PULL_REQUEST, REPOSITORY]) { totalCount } } }`;
  return octokit<{
    user: {
