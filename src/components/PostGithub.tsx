...
export interface PostGithubProps {
  repo: NonNullable<Query<'github.get-repo'>>;
}
...
export const PostGithub: FC<PostGithubProps> = ({ repo }) => (
  <Link href={repo.url} passHref>
    ...
    >
      ...
      <p className="text-2 font-semibold mt-4 mb-5">{repo.description}</p>
      <div className="flex gap-4 items-center">
        ...
        <div className="flex gap-2 items-center text-brand-2">
          ...
          <span className="text-2 font-semibold">{repo.stars}</span>
        </div>
        <div className="flex gap-2 items-center text-brand-2">
          <GithubForkIcon width={20} height={20} />
          <span className="text-2 font-semibold">{repo.forks}</span>
        </div>
      </div>
    </a>
  </Link>
